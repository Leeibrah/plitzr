...
if(isset($_POST['data'])) {
		...
		if(isset($_SESSION['cart_'.$Product_id]['no'])){
...
$quantity_of_all =0;
 ...
 if($quantity_of_all ==count($_SESSION) ) {
    // header('location: /facebookshop/shop/cart.php');
      }
//end
...
<div id="main_container" style="bgcolor:#FFF;">
	<div id="site">
		<header>
...
<nav>
	<ul>
		<li><a href="bobble-shop.php" class="js-loading">Home</a></li><li class="separator"><!-- --></li><li><a href="/upload/shop/cart.php" class="js-loading">1. Cart</a></li><li class="separator"><!-- --></li><li>2. Payment method</li><li class="separator"><!-- --></li><li>3. Review order</li><li class="separator"><!-- --></li><li>4. Confirmation</li>	</ul>
	<div class="clr"><!-- --></div>
</nav>
			...
			<p id="fanDiscountMsg">
				<span id="fanDiscountDynMsg">
					Click on the Like button to become a fan and get 10% off on your purchase. 
								...
								Hurry up, only the next 1 buyers will get the discount!	
			</p>
		</div>
...
<br />
							...
							<form data-unique-id="<?php  echo $value['product_id']; ?>" class="quantity-box" method="post" action="shoping_cart_form.php" accept-charset="utf-8">
							<div style="display:none;"><input type="hidden" name="_method" value="POST" ></div>							
							<input type="hidden" name="data[Shop][id]" value="<?php  echo $value['Shop_id']; ?>" id="ShopId_.<?php  echo $value['product_id']; ?>._0" />							
							<input type="hidden" name="data[Shop][slug]" value="<?php if(isset($value['shop_slug'])){ echo $value['shop_slug']; } ?>" id="ShopSlug_.<?php  echo $value['product_id']; ?>._0" />							
							...
							<input type="hidden" name="data[Product][quantity]" value="<?php if(isset($value['no'])){ echo $value['no']; } ?>" id="ProductQuantity_.<?php  echo $value['product_id']; ?>._0" />							
							...
							<input type="hidden" name="data[ProductVariation][index]" value="<?php  echo $value['ProductVariation_id']; ?>" id="ProductVariationIndex_.<?php  echo $value['product_id']; ?>._0" />							
								...
								<span class="quantity"><?php if(isset($value['no'])){ echo $value['no']; } ?></span>
							...
							<div class="price-box" style="bgcolor:#FFF;">
								<div class="price" style="bgcolor:#FFF;">KSH. <?php  echo $value['price'] ; ?></div>

								<form method="post" action="/shop/cart/remove-product" accept-charset="utf-8">
									<div style="display:none; bgcolor:#FFF;">
			...
			<div class="amount-box">
				...
				<div class="value" id="total">KSH. <?php echo total_amount(); ?></div>
				<div class="label">Total Price*</div>
				<div class="clr"><!-- --></div>

								<div class="value light">KSH. <?php $a = total_amount(); echo number_format(($a*19)/100,2); ?></div>
		...
		<div class="button-box">
			...
			<a href="shoping_cart_form.php" class="button js-loading">Pesapal</a>		
			</div>
	</div> <!-- /.fb-box (Cart) -->
